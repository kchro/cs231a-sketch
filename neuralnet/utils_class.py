...
class DataLoader(object):
  ...
  #gets fed "train_strokes" whole array
  def preprocess(self, strokes):
    """Remove entries from strokes having > max_seq_length points."""
    raw_data = []
    seq_len = [] # total stroke length (of all drawings)
    count_data = 0 #count of images that aren't removed
    new_labels = []
    
    for i in range(len(strokes)):
      #data = single drawing
      data = strokes[i]
      #removes images w/ too many strokes
      if len(data) <= (self.max_seq_length):
        count_data += 1
        # removes large gaps from the data
        #replaces all values over limit or under -limit with limit
        data = np.minimum(data, self.limit)
        data = np.maximum(data, -self.limit)
        data = np.array(data, dtype=np.float32)
        data[:, 0:2] /= self.scale_factor #divide data by scale_factor
  ...
  def sample(self, index):
    """Return a specific sample, in stroke-3 format as used by draw_strokes."""
    sample = np.copy(self.strokes[index])
    return sample
  ...
  def calculate_normalizing_scale_factor(self):
    ...
    for i in range(len(self.strokes)):
      #if the length of the drawing is > max seq length, skip
      if len(self.strokes[i]) > self.max_seq_length:
        continue
      #if not, then look at each parameter
