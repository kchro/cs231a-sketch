...
import model_class as sketch_rnn_model
...
def load_dataset(data_dir, model_params, inference_mode=False):
  ...
  #train_y = 0*len(train_strokes)=70,000
  for idx, dataset in enumerate(datasets): #datasets[0] = 0,dataset
    data_filepath = os.path.join(data_dir, dataset) #'final.npz'
    #load data online
  ...
  num_points = 0
  #calculate average length of strokes
  ...
  # calculate the max strokes of any drawing
...
def pred(sess, model, image, max_len):
  """Returns prediction for the image"""
  x = utils.pad_image(image, max_len)
  #returns (1,126,5) tensor
  feed = {model.input_data: x, model.y_labels: [0], model.sequence_lengths: [len(image)]}
